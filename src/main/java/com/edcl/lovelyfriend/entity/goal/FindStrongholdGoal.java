...
public class FindStrongholdGoal extends Goal {
    ...
    private static final TagKey<Structure> STRONGHOLD =
            TagKey.create(Registries.STRUCTURE, Identifier.fromNamespaceAndPath("lovelyfriend", "stronghold"));
    ...
    public boolean canUse() {
        ...
        targetPos = sl.findNearestMapStructure(STRONGHOLD, entity.blockPosition(), 128, false);
    ...
    public void tick() {
        ...
        if (cur.distanceToSqr(lastPos) < 0.05) {
            if (++stuckTimer > MAX_STUCK_TIME) { targetPos = null; cooldown = COOLDOWN_MIN; return; }
        } else {
            stuckTimer = 0;
        }
        lastPos = cur;
